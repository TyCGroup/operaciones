...
async function saveEventChanges() {
    ...
    try {
        ...
        const formData = {
            ...
            // Campos de compatibilidad (primera factura)
            numeroFactura: facturas[0]?.numeroFactura || '',
            subtotal: facturas[0]?.subtotal || 0,
            iva: facturas[0]?.iva || 0,
            total: facturas.reduce((sum, f) => sum + f.total, 0), // Total general
...
function closeEditModal() {
    ...
    // Limpiar contenedor de facturas
    document.getElementById('facturasContainer').innerHTML = '';
    facturaCounter = 1;
    
    // Mostrar botones en caso de que estuvieran ocultos
    const buttons = elements.editModal.querySelectorAll('.btn-add, .btn-remove-factura');
    buttons.forEach(btn => {
        btn.style.display = 'flex';
        btn.disabled = false;

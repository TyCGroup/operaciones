...
`;
    ...
    const inputs = elements.editModal.querySelectorAll('input, select, textarea, button.btn-add, button.btn-remove');
    inputs.forEach(input => {
        input.disabled = readOnly;
        if (readOnly && (input.classList.contains('btn-add') || input.classList.contains('btn-remove'))) {
...
function collectDynamicData() {
    ...
    document.querySelectorAll('.pendiente-input').forEach(input => {
        ...
        const concepto = document.querySelector(`input.concept-input[data-index="${index}"]`)?.value.trim() || '';
    ...
    document.querySelectorAll('.nopagado-input').forEach(input => {
        const value = parseFloat(input.value) || 0;
        const index = input.getAttribute('data-index');
        const concepto = document.querySelector(`#noPagadosContainer input.concept-input[data-index="${index}"]`)?.value.trim() || '';
    ...
    document.querySelectorAll('.pagado-input').forEach(input => {
        const value = parseFloat(input.value) || 0;
        const index = input.getAttribute('data-index');
        const concepto = document.querySelector(`#pagadosContainer input.concept-input[data-index="${index}"]`)?.value.trim() || '';
...
function loadDynamicData(event) {
    ...
    // Cargar montos no pagados y pagados de manera similar...
    // (código similar al de montos pendientes)
}

...
function updateResumenFinanciero() {
    ...
    // Calcular subtotal total e IVA total
    let subtotalTotal = 0;
    let ivaTotal = 0;
    let totalPagado = 0;
    let totalSinPagar = 0;
    ...
    const totalEvento = subtotalTotal + ivaTotal;
    const pendientePorFacturar = totalEvento - totalPagado - totalSinPagar;
    
    // Actualizar elementos del DOM
    if (elements.resumenSubtotal) {
        elements.resumenSubtotal.textContent = '$' + formatNumber(subtotalTotal);
    }
    if (elements.resumenIVA) {
        elements.resumenIVA.textContent = '$' + formatNumber(ivaTotal);
    }
